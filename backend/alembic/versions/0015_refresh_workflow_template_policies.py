"""refresh workflow template policies

Revision ID: 0015_refresh_workflow_template_policies
Revises: 0014_refresh_workflow_template_patterns
...
revision = "0015_refresh_workflow_template_policies"
down_revision = "0014_refresh_workflow_template_patterns"
branch_labels = None
depends_on = None
...
def downgrade() -> None:
    policy_prefixes = ("tier:", "risk:", "approval:", "tool:", "work-units:")
    bind = op.get_bind()
    for item in _load_templates():
        name = str(item["name"]).strip()
        ...
        bind.execute(
            ...
            .values(tags_json=json.dumps([tag for tag in tags if not tag.startswith(policy_prefixes)]))
        )
