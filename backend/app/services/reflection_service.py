...
def create_daily_reflection(
    db: Session, reflection_input: DailyReflectionInput, *, record_source: str = "user"
) -> DailyReflectionSavedResponse:
    logger.info(
    ...
    record = ReflectionEntry(
        entry_date=business_date_from_utc(created_at),
...
def list_daily_reflections(db: Session, *, include_system: bool = False) -> DailyReflectionHistoryResponse:
    query = db.query(ReflectionEntry)
    if not include_system:
        query = query.filter(~ReflectionEntry.record_source.in_(SYSTEM_RECORD_SOURCES))
    records = query.order_by(ReflectionEntry.created_at.desc(), ReflectionEntry.id.desc()).all()
